...
export interface SortableListProps {
  /** One per item, stable across renders and unique within this list. */
  ids: string[]
...
export function SortableList({ ids, onReorder, children, label }: SortableListProps) {
  ...
  return (
    <DndContext
      ...
      onDragEnd={handleEnd}
      accessibility={label ? { announcements: announcementsFor(label) } : undefined}
    >
...
function announcementsFor(label: string) {
  return {
    onDragStart: ({ active }: { active: { id: string | number } }) =>
      `Picked up ${active.id} in ${label}. Use the arrow keys to move it, space to drop it.`,
    onDragOver: () => undefined,
    onDragEnd: ({ active }: { active: { id: string | number } }) => `Dropped ${active.id}.`,
    onDragCancel: ({ active }: { active: { id: string | number } }) =>
      `Left ${active.id} where it was.`,
  }
}
